...
const MAX_SLOTS_PER_DAY = 3;
        ...
        function OverviewComponent({ onShowToast, onNavigate, mode, session }) {
            const isPink = mode === 'makeup' || (!mode && session?.role === 'makeup');
            const isGreen = mode === 'dekor' || (!mode && session?.role === 'dekor');
            const isBlue = mode === 'studio' || (!mode && session?.role === 'studio');
...
_Pesan ini adalah pesan otomatis dan hanya dikirimkan melalui Whatsapp resmi LAPANBELAS ID._`;
        ...
        function AppointmentComponent({ onShowToast, initialFilter, session, mode }) {
            const isMakeup = mode === 'makeup' || session?.role === 'makeup';
            const isStudio = mode === 'studio';
            const isDecor = mode === 'dekor' || session?.role === 'dekor';
            ...
            const parseNotesField = (notesStr) => {
                ...
                if (!notesStr) return { prewedDate, selectedAddonNames, voucherCode, customFees, keterangan, div, namaPria, namaWanita, jamSesi, roomStudio, photographer, durasiSesi, jadwalFitting, jadwalSurvei, jadwalPemasangan };
                ...
                return { prewedDate, selectedAddonNames, voucherCode, customFees, keterangan, div, namaPria, namaWanita, jamSesi, roomStudio, photographer, durasiSesi, jadwalFitting, jadwalSurvei, jadwalPemasangan };
            };
            ...
            React.useEffect(() => {
                ...
                const computedTotal = Math.max(0, basePrice + addonsPrice + customFeesPrice - voucherDiscount);
                setFormData(prev => ({ ...prev, total: computedTotal }));
            }, [formData.pkg, selectedAddonIds, customFees, selectedVoucherCode, packages, addonsList, vouchers, isModalOpen]);

            const handleAddClick = () => {
                setEditId(null);
                setDivisi(isMakeup ? 'Lady Makeup' : isStudio ? 'Studio Lapanbelas' : isDecor ? 'Lapanbelas Dekorasi' : 'lapanbelas.id');
                ...
                setFormData({ ...defaultForm, pkg: defaultPkg?.title || '' });
            ...
            const handleSubmit = async (e) => {
                ...
                if (divisi === 'Studio Lapanbelas') {
                    if (formData.jamSesi) formattedNotes += `[JAM SESI]: ${formData.jamSesi}\n`;
                ...
                const dbPayload = {
                    ...
                    jam_akad: formData.jamAkad || null,
                                        ...
                                        <tr key={idx} className="hover:bg-white/5 transition">
                                            ...
                                            <td className="px-6 py-4">
                                                <span className={`px-2 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider ${
                                                    apt.division === 'lapanbelas.id' ? 'bg-pink-500/20 text-pink-400' :
                                                    apt.division === 'Studio Lapanbelas' ? 'bg-blue-500/20 text-blue-400' :
                                                    apt.division === 'Lady Makeup' ? 'bg-purple-500/20 text-purple-400' :
                                ...
                                <form onSubmit={handleSubmit} className="space-y-4">
                                    ...
                                    <div className="bg-white/3 border border-white/8 rounded-xl p-4 space-y-3 mb-4">
                                        ...
                                        <select required disabled={isMakeup || isStudio || isDecor} value={divisi} onChange={e => {
                                            ...
                                            setFormData(prev => ({ ...prev, pkg: '' })); // reset paket saat divisi berubah
                                                ...
                                                <div className="grid grid-cols-3 gap-3">
                                                    ...
                                                    <div>
                                                        ...
                                                        <select value={formData.photographer} onChange={e => setFormData({ ...formData, photographer: e.target.value })} className="w-full bg-gray-900 border border-white/10 rounded-lg px-3 py-2 text-sm outline-none focus:border-blue-500 text-white appearance-none">
                                                            ...
                                                            {staffList.filter(s => s.role === 'admin').map((s, i) => (
                                                                <option key={i} value={s.display_name}>{s.display_name}</option>
                                                            ))}
                                                        </select>
                                                    </div>
                                                    <div>
                            ...
                            <div className="glass-panel border border-orange-500/30 p-6 rounded-2xl w-full max-w-md text-center animate-in zoom-in-95 duration-200">
                                ...
                                <div className="space-y-2 mb-5 text-left max-h-[200px] overflow-y-auto custom-scrollbar">
                                    {collisionWarnings.map((w, i) => (
                                        <div key={i} className={`flex items-start gap-2.5 p-3 rounded-xl text-[11px] leading-relaxed border ${
                                            w.type === 'room'
        ...
        function PricelistComponent({ onShowToast, session, mode }) {
            const isMakeupMode = mode === 'makeup' || session?.role === 'makeup';
            const isStudioMode = mode === 'studio' || session?.role === 'studio';
            const isDecorMode = mode === 'dekor' || session?.role === 'dekor';
            const [packages, setPackages] = React.useState([]);
            const [isModalOpen, setIsModalOpen] = React.useState(false);
            const [editId, setEditId] = React.useState(null);
            const [confirmDeleteId, setConfirmDeleteId] = React.useState(null);
                            ...
                            <div className="glass-panel border border-white/10 p-6 rounded-2xl w-full max-w-md relative animate-in zoom-in-95">
                                ...
                                <form onSubmit={handleSubmit} className="space-y-4">
                                    ...
                                    <div>
                                        ...
                                        <select required value={formData.role} onChange={e => setFormData({ ...formData, role: e.target.value })} className="w-full bg-gray-900 border border-white/10 rounded-lg px-3 py-2 text-sm outline-none text-white appearance-none">
                                            <option value="owner">Owner (Full Akses)</option>
                                            <option value="karyawan">Karyawan (Akses Foto & Video)</option>
                                            <option value="editor_foto">Editor Foto Khusus</option>
                                            <option value="editor_video">Editor Video Khusus</option>
                                            <option value="makeup">Lady Makeup (Divisi Rias)</option>
                                            <option value="studio">Studio Lapanbelas (Divisi Studio)</option>
                                            <option value="dekor">Lapanbelas Dekorasi (Divisi Dekorasi)</option>
                                        </select>
                                    </div>
                                    <div className="pt-4 flex gap-3">
                                        <button type="button" onClick={() => setIsModalOpen(false)} className="flex-1 py-2.5 rounded-xl border border-white/10 text-sm font-medium hover:bg-white/5 transition">Batal</button>
        ...
        function JadwalRiasComponent({ onShowToast, session }) {
            ...
            const [pinggang, setPinggang] = React.useState('');
            const [pinggul, setPinggul] = React.useState('');
            ...
            const handleOpenChecklistModal = (appt) => {
                ...
                setLd(checklist.ld || '');
                setPinggang(checklist.pinggang || '');
                setPinggul(checklist.pinggul || '');
                setTinggi(checklist.tinggi || '');
            ...
            const handleSaveChecklist = async (e) => {
                ...
                try {
                    ...
                    const checklistObj = { busana, aksesoris, catatanRias, ld, pinggang, pinggul, tinggi };
                                                ...
                                                <button 
                                                    onClick={async () => {
                                                        try {
                                                            ...
                                                            a.href = url;
                                                            a.download = `Fitting_Busana_${appt.client_name.replace(/\\s+/g, '_')}_${appt.id}.pdf`;
                                ...
                                <form onSubmit={handleSaveChecklist} className="space-y-4">
                                    <div className="grid grid-cols-2 gap-4">
                                        ...
                                        <div className="col-span-2 border-t border-white/5 pt-3">
                                            <p className="text-xs font-semibold text-pink-400 mb-2 uppercase tracking-wider">📏 Ukuran Badan (Alterasi / Custom)</p>
                                        </div>
                                        <div>
                                            <label className="text-[11px] text-gray-400 block mb-1">Lingkar Dada (LD) - cm</label>
                                            <input type="text" placeholder="Cth: 90 cm" value={ld} onChange={e => setLd(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-xs outline-none focus:border-pink-500 text-white" />
                                        </div>
                                        <div>
                                            <label className="text-[11px] text-gray-400 block mb-1">Lingkar Pinggang - cm</label>
                                            <input type="text" placeholder="Cth: 75 cm" value={pinggang} onChange={e => setPinggang(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-xs outline-none focus:border-pink-500 text-white" />
                                        </div>
                                        <div>
                                            <label className="text-[11px] text-gray-400 block mb-1">Lingkar Pinggul - cm</label>
                                            <input type="text" placeholder="Cth: 98 cm" value={pinggul} onChange={e => setPinggul(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-xs outline-none focus:border-pink-500 text-white" />
                                        </div>
                                        <div>
                                            <label className="text-[11px] text-gray-400 block mb-1">Tinggi Badan / Pj Baju</label>
                                            <input type="text" placeholder="Cth: 135 cm" value={tinggi} onChange={e => setTinggi(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-xs outline-none focus:border-pink-500 text-white" />
                                        </div>
                                                ...
                                                <div key={b.id} className="p-3 bg-[#111111] hover:bg-[#1a1a1a] border border-white/10 rounded-xl transition flex flex-col shadow-md">
                                                    <div className="flex justify-between items-start mb-2">
                                                        ...
                                                        <span className={`text-[9px] px-2 py-0.5 rounded-full font-bold uppercase ${
                                                            b.status === 'Lunas' ? 'bg-green-500/20 text-green-400' :
                                ...
                                {photographers.map(p => {
                                    ...
                                    return (
                                        <div
                                            ...
                                            className={`border rounded-xl p-5 flex flex-col justify-between transition-all ${!isTetap ? 'cursor-pointer hover:border-blue-500/50 hover:bg-blue-500/5' : ''} ${
                                                isSelected
                                        ...
                                        >
                                            <div>
                                                ...
                                                <span className={`text-[10px] px-2.5 py-1 rounded-full font-bold tracking-wider ${
                                                    isTetap ? 'bg-purple-500/20 text-purple-400 border border-purple-500/30' : 'bg-orange-500/20 text-orange-400 border border-orange-500/30'
                                                }`}>
                                                    {p.role}
                                                </span>
                                            </div>
                ...
                try {
                    ...
                    if (data) {
                        const parsed = data.map(appt => {
                            ...
                            const roomMatch = notesStr.match(/\[ROOM STUDIO\]:\s*([^\n]+)/);
                            if (roomMatch) roomStudio = roomMatch[1].trim();
                    ...
                    <div key={appt.id} className="glass-panel border border-white/10 hover:border-white/20 transition rounded-2xl p-4 flex flex-col 2xl:flex-row gap-4 justify-between items-center">
                            ...
                            <div>
                                <div className="flex flex-wrap items-center gap-2 mb-1">
                                    ...
                                    <span className={`text-[9px] px-2 py-0.5 rounded-full font-bold uppercase shrink-0 ${
                                        st === 'Aktif' ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30' :
                        ...
                        <div className="flex gap-2 w-full 2xl:w-auto overflow-x-auto pb-2 2xl:pb-0 hide-scrollbar">
                            <button 
                                ...
                                className={`px-3 py-1.5 lg:px-4 lg:py-2 rounded-xl text-[10px] lg:text-xs font-bold transition border shrink-0 ${
                                    st === 'Menunggu' 
                            ...
                            <button 
                                ...
                                className={`px-3 py-1.5 lg:px-4 lg:py-2 rounded-xl text-[10px] lg:text-xs font-bold transition border flex items-center gap-1 shrink-0 ${
                                    st === 'Aktif' 
        ...
        function AdminDashboard() {
                    ...
                    if (saved) {
                        try {
                            ...
                            if (sbSession) {
                                ...
                                if (parsedSession.role === 'karyawan' || parsedSession.role === 'editor_foto') {
                                    setActiveMenu('assign-foto');
                                } else if (parsedSession.role === 'editor_video') {
                                    setActiveMenu('assign-video');
                                } else if (parsedSession.role === 'makeup') {
                                    setActiveMenu('overview-makeup');
                                }
                            } else {
            ...
            const handleLogin = (sessData) => {
                ...
                setSession(sessData);
                if (sessData.role === 'karyawan' || sessData.role === 'editor_foto') setActiveMenu('assign-foto');
                else if (sessData.role === 'editor_video') setActiveMenu('assign-video');
                else if (sessData.role === 'makeup') setActiveMenu('overview-makeup');
                else if (sessData.role === 'studio') setActiveMenu('overview-studio');
            ...
            const visibleMenus = session.role === 'makeup'
                ? [
                    { id: 'overview-makeup', label: 'Overview', icon: 'layout-dashboard' },
                    { id: 'appointment-makeup', label: 'Appointment', icon: 'calendar-days' },
                    { id: 'jadwal-rias', label: 'Jadwal Rias', icon: 'wand-sparkles' },
                ...
                ? menus.filter(m => m.id === 'assign-foto' || m.id === 'assign-video' || m.id === 'assign-foto-studio')
                : session.role === 'editor_foto'
                ? menus.filter(m => m.id === 'assign-foto' || m.id === 'assign-foto-studio')
                : session.role === 'editor_video'
                ? menus.filter(m => m.id === 'assign-video')
                : session.role === 'owner'
                ? menus
                : menus.filter(m => m.id !== 'users'); // Only owner sees users menu
                    ...
                    <aside className={`w-64 bg-[#0f1115] border-r border-white/10 flex flex-col shrink-0 fixed inset-y-0 left-0 z-50 transform transition-transform duration-300 md:relative md:translate-x-0 ${isSidebarOpen ? 'translate-x-0 shadow-2xl' : '-translate-x-full'}`}>
                        <div className="p-6 border-b border-white/10">
                            <h1 className="text-xl font-bold tracking-wider uppercase">
                                {session.role === 'studio' ? 'Studio Lapanbelas' : session.role === 'dekor' ? 'Lapanbelas Dekorasi' : session.role === 'makeup' ? 'Lady Makeup' : '18Studio Admin'}
                            </h1>
                        ...
                        <nav className="flex-1 overflow-y-auto py-4 custom-scrollbar">
                            <ul className="space-y-1">
                                {visibleMenus.map(menu => {
                                            ...
                                            return (
                                                <li key={menu.id} className="flex flex-col">
                                                    <button
                                                        ...
                                                        className={`w-full flex items-center justify-between px-6 py-3 text-sm text-left transition-all ${
                                                            isAnySubActive 
                                                            ...
                                                            {makeupSubmenus.map(sub => (
                                                                <li key={sub.id}>
                                                                    <button
                                                                        ...
                                                                        className={`w-full flex items-center gap-3 pl-6 pr-4 py-2 text-xs text-left transition-all ${
                                                                            activeMenu === sub.id
                                    ...
                                    const isThemePink = session?.role === 'makeup' || isMakeupActive;
                                    ...
                                    const isThemeBlue = session?.role === 'studio' || isStudioActive;
                                    ...
                                    const isThemeGreen = session?.role === 'dekor' || isDecorActive;
                        ...
                        <div className="flex-1 overflow-y-auto p-6 z-10">
                            {(() => {
                                switch (activeMenu) {
                                    ...
                                    default: return <OverviewComponent onShowToast={showToast} session={session} />;
                                }
                            })()}
                        </div>
                    </main>
